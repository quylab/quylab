...
function simplify($arr){
	//return arr.map(r => r.map(c => Array.isArray(c)&&c.length===1?c[0]:c))
...
function hassingletons($arr){ 
	//return arr.some(r => r.some(c => Array.isArray(c)&&c.length===1)) 
...
function load_in($sudokuArr){
	$cell_string = '';
	/* br(2);
	echo 'sudarr';
	varjson($sudokuArr);
	br(2); */
	if(!is_array($sudokuArr)){ die('returned false'); }
	foreach($sudokuArr as $row){
		$cell_string.='<div class="row">';
		foreach($row as $cell){
			$cell_ = $cell==false?'':$cell;
			$cell_string .= '<div class="cell">'.$cell_.'</div>';
		}
		$cell_string.='</div>';
	}
	return $cell_string;
...
function output($arr){
	echo('<div class="wrapper">'.load_in($arr).'</div>');
}
output($result);
?><!DOCTYPE html>
<html lang="en-us">
